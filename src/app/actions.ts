...
async function requireDbUser(callbackPath?: string) {
...
export async function joinGroup(token: string) {
  const user = await requireDbUser(`/join/${token}`);
  const invite = await prisma.inviteLink.findUnique({ where: { token } });
  if (!invite) redirect("/groups?invite=invalid");
  if (invite.expiresAt && invite.expiresAt < new Date()) redirect("/groups?invite=expired");
